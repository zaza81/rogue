...
trait LiftRogue extends Rogue {
  ...
  implicit def rbsonRecordFieldToBsonRecordQueryField[
    ...
    if (f.name.indexOf('.') >= 0) {
      ...
      val rec: B = typedField.setFromJValue(JArray(JInt(0) :: Nil)).openOrThrowException("Rogue hack failed").head
      new BsonRecordQueryField[M, B](f, _.asDBObject, rec)
    } else {
  ...
  implicit def bsonRecordListFieldToBsonRecordListQueryField[
    ...
    val rec = f.setFromJValue(JArray(JInt(0) :: Nil)).openOrThrowException("Rogue hack failed").head // a gross hack to get at the embedded record
